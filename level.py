...
def handler(str_msg):
    ...
    if msg_dict['action'] == 'set':
        if not msg_dict.has_key('value'):
            ...
            return
        if msg_dict['value']:
            # code for setting level to needed value
            ack_value(DID, some_value)
            return
...
mqtt_con = MQTTSub("168.63.82.20", USER_ID + "thing/" + DID, handler)
